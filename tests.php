<?php
    require("jsondb.php");
    $database = new jsonDB\db("data.json"); # While calling the class, database file must be entered as parameter.
    try{
        if(!empty($database->get()["available"]))
            echo("JSONDB is active.");
        else
            die("JSONDB error occurred.");
        # In case of any error, all of the following will return an error code. For more information: https://www.php.net/manual/en/language.exceptions.php
        # Important Notes:
        #Decode will return an error code if any of the write, read operations fail.
        ...
        echo($e->errorMsg()); # If an error occurs...
    }
?>